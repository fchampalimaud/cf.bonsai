...
namespace CF.Scripting.Python
{
    ...
    public class GetDoubleArray : Source<double[]>
    {
        /// <summary>
        ...
        public override IObservable<double[]> Generate()
        {
            return RuntimeManager.RuntimeSource.SelectMany(runtime =>
            {
                ...
                return Observable.Return(module.Get(VariableName).As<double[]>());
            });
        }
        ...
        public IObservable<double[]> Generate<TSource>(IObservable<TSource> source)
        {
            return RuntimeManager.RuntimeSource.SelectMany(runtime =>
            {
                return source.Select(_ =>
                {
                    ...
                    {
                        var module = Module ?? runtime.MainModule;
                        return module.Get(VariableName).As<double[]>();
                    }
                });
            });
        }
        ...
        public IObservable<double[]> Process(IObservable<RuntimeManager> source)
        {
            return source.Select(runtime =>
            {
                using (Py.GIL())
                {
                    return runtime.MainModule.Get(VariableName).As<double[]>();
                }
            });
        }
    }

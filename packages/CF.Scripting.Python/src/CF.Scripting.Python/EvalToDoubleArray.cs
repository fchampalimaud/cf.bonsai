...
namespace CF.Scripting.Python
{
    ...
    public class EvalToDoubleArray : Combinator<double[]>
    {
        /// <summary>
        ...
        /// </returns>
        public override IObservable<double[]> Process<TSource>(IObservable<TSource> source)
        {
            return RuntimeManager.RuntimeSource.SelectMany(runtime =>
            {
                return source.Select(_ =>
                {
                    using (Py.GIL())
                    {
                        ...
                        return module.Eval(Expression).As<double[]>();
                    }
                });
            });
        }
        ...
        /// of evaluating the Python expression.
        /// </returns>
        public IObservable<double[]> Process(IObservable<PyModule> source)
        {
        ...
        /// of evaluating the Python expression.
        /// </returns>
        public IObservable<double[]> Process(IObservable<RuntimeManager> source)
        {
            return source.Select(runtime =>
            {
                using (Py.GIL())
                {
                    return runtime.MainModule.Eval(Expression).As<double[]>(); ;
                }
            });
        }
    }

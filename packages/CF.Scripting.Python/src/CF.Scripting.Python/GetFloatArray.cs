...
namespace CF.Scripting.Python
{
    ...
    public class GetFloatArray : Source<float[]>
    {
        /// <summary>
        ...
        public override IObservable<float[]> Generate()
        {
            return RuntimeManager.RuntimeSource.SelectMany(runtime =>
            {
                ...
                return Observable.Return(module.Get(VariableName).As<float[]>());
            });
        }
        ...
        public IObservable<float[]> Generate<TSource>(IObservable<TSource> source)
        {
            return RuntimeManager.RuntimeSource.SelectMany(runtime =>
            {
                return source.Select(_ =>
                {
                    ...
                    {
                        var module = Module ?? runtime.MainModule;
                        return module.Get(VariableName).As<float[]>();
                    }
                });
            });
        }
        ...
        public IObservable<float[]> Process(IObservable<RuntimeManager> source)
        {
            return source.Select(runtime =>
            {
                using (Py.GIL())
                {
                    return runtime.MainModule.Get(VariableName).As<float[]>();
                }
            });
        }
    }

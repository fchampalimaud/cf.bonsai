...
namespace CF.Scripting.Python
{
    ...
    public class EvalToFloatArray : Combinator<float[]>
    {
        /// <summary>
        ...
        /// </returns>
        public override IObservable<float[]> Process<TSource>(IObservable<TSource> source)
        {
            return RuntimeManager.RuntimeSource.SelectMany(runtime =>
            {
                return source.Select(_ =>
                {
                    using (Py.GIL())
                    {
                        ...
                        return module.Eval(Expression).As<float[]>();
                    }
                });
            });
        }
        ...
        /// of evaluating the Python expression.
        /// </returns>
        public IObservable<float[]> Process(IObservable<PyModule> source)
        {
        ...
        /// of evaluating the Python expression.
        /// </returns>
        public IObservable<float[]> Process(IObservable<RuntimeManager> source)
        {
            return source.Select(runtime =>
            {
                using (Py.GIL())
                {
                    return runtime.MainModule.Eval(Expression).As<float[]>(); ;
                }
            });
        }
    }
